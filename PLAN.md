...
- **Phase 4 — Captions** ✅ *(done)*: `buildCaptionCues` from the EDL (+ tests); synced overlay on the preview with 3 style presets (Bold/Clean/Boxed) + color; enable toggle in the captions panel.
- **Phase 5 — User media** 🟢 *(music + images verified; b-roll deferred)*: background music (bg track + volume, mixed under speech via `amix`) and image/logo overlays (3×3 position grid, scaled + `overlay`). Folded into `edl.tracks` at export; shown in the preview (overlays + synced music). **Verified in a real browser** (`npm run test:e2e:media`): a logo lands in the exported pixels (24k magenta px in its region) and music adds an audio stream. **Deferred:** inserting extra b-roll video as EDL segments.
- **Phase 6 — Export via `Renderer`** 🟢 *(verified, incl. caption burn-in)*: FFmpeg.wasm deps + `WasmRenderer` (dynamic-imported); pure `ffmpegArgsForExport` (cut + concat + scale/crop to aspect ratio, audio-aware, optional caption burn) with tests; 9:16/1:1/16:9 presets; progress bar; download link; `estimateRenderCost` wired. **Caption burn-in done:** EDL cues → output-time SRT + ASS `force_style`, burned via the `subtitles` filter with a bundled Roboto TTF (libass). **Verified end-to-end in a real browser** (Playwright): both audio and silent clips export valid MP4s (`npm run test:e2e`); an A/B pixel check (`npm run test:e2e:captions`) confirms captions are in the exported pixels (ON: ~15k white px in the caption band, OFF: 0). **Remaining:** mix music / overlay images (after Phase 5), longer-clip testing → the `ServerRenderer` signal.
- **Phase 7 — Polish** 🟢 *(filters + layout + onboarding done)*: 6 color "looks" (None/Warm/Cool/Mono/Vivid/Bright) with a CSS preview + FFmpeg export filter, verified in pixels (`npm run test:e2e:filter` — Mono → 100% grayscale); responsive layout (panels stack on narrow screens); state-aware 3-step onboarding hint. **Remaining:** transitions, deeper empty/error states.
