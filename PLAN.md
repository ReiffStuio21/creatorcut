...
- **Phase 6 — Export via `Renderer`** 🟢 *(core verified — caption burn-in pending)*: FFmpeg.wasm deps + `WasmRenderer` (dynamic-imported); pure `ffmpegArgsForExport` (cut + concat + scale/crop to aspect ratio, audio-aware) with tests; 9:16/1:1/16:9 presets; progress bar; download link; `estimateRenderCost` wired. **Verified end-to-end in a real browser** (Playwright, `npm run test:e2e`): both audio and silent clips export a valid MP4; the renderer probes for an audio stream so silent clips don't abort. The bundled core has libass/libfreetype, so caption burn-in is feasible. **Remaining:** burn captions into the file, mix music / overlay images (after Phase 5), longer-clip testing → the `ServerRenderer` signal.
