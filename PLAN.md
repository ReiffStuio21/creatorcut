...
- **Phase 6 — Export via `Renderer`** 🟡 *(core done — caption burn-in pending)*: FFmpeg.wasm deps + `WasmRenderer` (dynamic-imported); pure `ffmpegArgsForExport` (cut + concat + scale/crop to aspect ratio) with tests; 9:16/1:1/16:9 presets; progress bar; download link; `estimateRenderCost` wired into the panel. **Remaining:** burn captions into the file (needs a libass-enabled core + fonts), mix music / overlay images (after Phase 5), and real-clip browser testing (30s → 2min) → the `ServerRenderer` signal.
